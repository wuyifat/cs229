function dt = traindtw(X,Y,alpha,maxdepth,splitfn,varargin)
...
split = splitfn(X,Y,alpha,maxdepth<1,varargin{:});
if (length(split)>1) 
	var = split(1);
	thresh = split(2);
	left = traindtw(X(X(:,var)<thresh,:),Y(X(:,var)<thresh),alpha(X(:,var)<thresh),maxdepth-1,splitfn,varargin{:});
	right = traindtw(X(X(:,var)>=thresh,:),Y(X(:,var)>=thresh),alpha(X(:,var)>=thresh),maxdepth-1,splitfn,varargin{:});
	dt = {split,left,right};
else
	dt = split;
end;
